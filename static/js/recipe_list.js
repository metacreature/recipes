...
var create_slideshow = function() {
    slideshow('#recipe_list a.recipe_entry', '/recipes/list/get', 'recipe_id', function(data, node) {
        data = data.data;
        var writeIngredients = function(persons) {
            ...
            var html = '<table>';
            for (row of data.ingredients_list) {
                var quantity = format_quantity(row.quantity * factor);
                html += '<tr>';
                if (has_alternative) {
                    html += '<td>' + (row.is_alternative ? recipe_detail_ingredients_alternative + ' ' : '') + '</td>';
                }
                html += '<td class="quantity">' + quantity + '</td>';
                html += '<td>' + row.unit_name + '</td>';
                html += '<td>' + row.ingredients_name + '</td>';
                html += '</tr>';
            }
            html += '</table>';
            node.find('.ingredients_list').html(html);
        }
        ...
        html += '<div class="recipe_name">' + data.recipe_name + '</div>';
        html += '<div class="tags">';
        html += data.category_name;
        if (data.tag_list.length) {
            html += ', ' + data.tag_list.join(', ');
        }
        html += '</div>';
        html += '<div class="user_name">@ ' + data.user_name + '</div>';
        ...
        html +=         '</div>';
        html +=         '<div class="ingredients_list"></div>';
        html +=      '</div>';
        html +=      '<div class="clear"></div>';
        html += '</div>';
        ...
        for(step of data.step_list) {
            cnt++;
            html += '<tr><td>#' + cnt + '</td><td>' + step + '</td></tr>';
        }
        html += '</table>';
        html += '</div>';
        node.html(html);

        writeIngredients(data.persons);
        node.find('select').on('click', function() { 
            writeIngredients($(this).val());
        });
    });
}
...
$('.filter_form').ajax_form({
    'success': function(form, button, data) {
        if (data.success) {
            ...
            history.replaceState({}, '', self.location.href.replace(/\?.*$/, '') + (query_string ? '?' + query_string : ''));
        }
